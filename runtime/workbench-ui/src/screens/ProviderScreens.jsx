...
export function Chat({ data, actor, onChanged, liveRevision }) {
  ...
  return <><PageHead title="Chat" subtitle="Slack through the real Web API. Read and post as a world person." command="Slack Web API"/>
    <div className="panel channel-layout"><nav className="channel-rail">{channels.map((item) => <button className={item.id === channel ? "active" : ""} key={item.id} onClick={() => setChannel(item.id)}># {item.name}</button>)}</nav>
...
export function Gmail({ data, actor, onChanged }) {
  ...
  return <><PageHead title="Gmail" subtitle="Google Workspace mail through the Gmail API." command="Google Gmail API"/>
    <Panel title={`Compose as ${actor.name}`} tools={<code className="blue">messages.send</code>}><form id="gmail-compose" className="action-form compose-form" onSubmit={submit}><label>TO<input name="to" type="email" required value={draft.to} onChange={edit("to")}/></label><label>SUBJECT<input name="subject" required value={draft.subject} onChange={edit("subject")}/></label><label>MESSAGE<textarea name="text" required value={draft.text} onChange={edit("text")} placeholder="Write a Gmail message…"/></label><div><Button kind="primary" disabled={action.busy}>{action.busy ? "Sending…" : draft.in_reply_to ? "Send reply" : "Send message"}</Button>{draft.in_reply_to && <Button type="button" onClick={() => setDraft({ to: "", subject: "", text: "" })}>Cancel reply</Button>}</div><ActionResult result={action.result}/></form></Panel>
    <div className="section"><Panel title="Gmail mailbox · 20 newest" tools={<FolderTabs folder={folder} setFolder={setFolder} inboxCount={folders.inbox?.resultSizeEstimate ?? 0} sentCount={folders.sent?.resultSizeEstimate ?? 0}/>}><div className="data-row mail-columns table-head"><span>MESSAGE</span><span>{folder === "sent" ? "TO" : "FROM"}</span><span>DATE</span><span></span></div>{mailbox.messages?.length ? mailbox.messages.map((message) => <div className="data-row mail-columns" key={message.id}><span><strong>{message.subject ?? "Gmail message"}</strong><small>{message.threadId}</small></span><code className="muted truncate">{folder === "sent" ? message.to : message.from}</code><span>{message.date ?? "—"}</span><Button kind="small" onClick={() => reply(message)}>Reply</Button></div>) : <div className="empty">This Gmail folder is empty.</div>}{mailbox.resultSizeEstimate > mailbox.messages.length && <div className="list-more muted">Showing the 20 newest. {mailbox.resultSizeEstimate - mailbox.messages.length} older messages are hidden.</div>}</Panel></div>
  </>;
}
...
export function Mail({ data, actor, onChanged }) {
  ...
  return <><PageHead title="Mail" subtitle="Standalone RFC mail. Send through SMTP and read through IMAP." command="SMTP submission · IMAP4rev1"/>
    <Panel title={`Compose as ${actor.name}`} tools={<code className="blue">SMTP submission</code>}><form id="smtp-compose" className="action-form compose-form" onSubmit={submit}><label>TO<input name="to" type="email" required value={draft.to} onChange={edit("to")}/></label><label>SUBJECT<input name="subject" required value={draft.subject} onChange={edit("subject")}/></label><label>MESSAGE<textarea name="text" required value={draft.text} onChange={edit("text")} placeholder="Write a mail message…"/></label><div><Button kind="primary" disabled={action.busy}>{action.busy ? "Sending…" : draft.in_reply_to ? "Send reply" : "Send through SMTP"}</Button>{draft.in_reply_to && <Button type="button" onClick={() => setDraft({ to: "", subject: "", text: "" })}>Cancel reply</Button>}</div><ActionResult result={action.result}/></form></Panel>
    <div className="section"><Panel title="Mailboxes through IMAP · 20 newest" tools={<FolderTabs folder={folder} setFolder={setFolder} inboxCount={folders.inbox?.exists ?? 0} sentCount={folders.sent?.exists ?? 0}/>}><div className="data-row mail-columns table-head"><span>MESSAGE</span><span>{folder === "sent" ? "TO" : "FROM"}</span><span>DATE</span><span></span></div>{mailbox.messages?.length ? mailbox.messages.map((message) => <div className="data-row mail-columns" key={message.seq}><span><strong>{message.headers.subject ?? "Mail message"}</strong><small>sequence {message.seq} · IMAP</small></span><code className="muted truncate">{folder === "sent" ? message.headers.to : message.headers.from}</code><span>{message.headers.date ?? "—"}</span><Button kind="small" onClick={() => reply(message)}>Reply</Button></div>) : <div className="empty">This IMAP folder is empty.</div>}{mailbox.exists > mailbox.messages.length && <div className="list-more muted">Showing the 20 newest. {mailbox.exists - mailbox.messages.length} older messages are hidden.</div>}</Panel></div>
  </>;
}
...
export function Files({ data, actor, onChanged }) {
  ...
  return <><PageHead title="Files" subtitle="Standalone SeaweedFS through its S3-compatible API." command="S3 ListObjectsV2 · PutObject"/>
    <Panel title="Buckets and objects"><div className="data-row resource-columns table-head"><span>BUCKET</span><span>OBJECT</span><span>SIZE</span><span>STATE</span></div>{buckets.flatMap((bucket) => (bucket.objects.length ? bucket.objects : [{ key: "No objects", size: "" }]).map((object) => <div className="data-row resource-columns" key={`${bucket.name}/${object.key}`}><strong>{bucket.name}</strong><code className="muted truncate">{object.key}</code><code>{object.size || "—"}</code><code className="green">available</code></div>))}</Panel>
...
export function Notion({ data, onChanged }) {
  ...
  return <><PageHead title="Notion" subtitle="Notion REST, MCP, OAuth, webhooks, Agent API, and selected Admin surfaces use one fixture state." command="REST 2026-03-11 · MCP 2025-11-25 · Admin 2026-06-01"/>
    ...
    <Panel title="Workbench support and evidence" tools={<code className="blue">Visible coverage, not a completeness claim</code>}>
    ...
    <Panel title={`Pages · ${notion.pages.length}`} tools={<code className="blue">POST /v1/search</code>}>
      <div className="data-row resource-columns table-head"><span>PAGE</span><span>ID</span><span>UPDATED</span><span>STATE</span></div>
      {notion.pages.map((page) => <div className="data-row resource-columns" key={page.id}><strong>{notionTitle(page)}</strong><code className="muted truncate">{page.id}</code><span>{page.last_edited_time}</span><code className={page.in_trash ? "yellow" : "green"}>{page.in_trash ? "in trash" : "available"}</code></div>)}
...
export function Website({ data }) {
  return <><PageHead title="Website" subtitle="The selected HTTP target and its current state." command="HTTP"/><Panel className="panel-pad"><strong>Current response</strong><p className="muted">{data.providers.website.preview}</p><div className="inline-actions"><Button kind="primary" onClick={() => window.open(data.bindings.SITE_BASE_URL, "_blank")}>Open HTTP target</Button><code className="muted">{data.bindings.SITE_BASE_URL}</code></div></Panel></>;
}
