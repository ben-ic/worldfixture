...
export function Gmail({ actor, onChanged, onAction }) {
  ...
  return <><PageHead title="Gmail" subtitle="Google Workspace mail through the Gmail API." command="Google Gmail API"/>
    ...
    <Panel title={`${folders.email} · 20 newest`} tools={<FolderTabs folder={folder} setFolder={setFolder} inboxCount={folders.inbox?.resultSizeEstimate ?? 0} sentCount={folders.sent?.resultSizeEstimate ?? 0}/>}>{mailbox.messages?.length ? mailbox.messages.map((message) => <ContentDetails key={`${actor.id}:${message.id}`} endpoint={`/api/provider/gmail-message?person_id=${encodeURIComponent(actor.id)}&id=${encodeURIComponent(message.id)}`} onReply={() => reply(message)} summary={<><span><strong>{message.subject ?? "Gmail message"}</strong><small>{folder === "sent" ? message.to : message.from}</small></span><time>{message.date ?? "—"}</time></>}/>) : <div className="empty">This Gmail folder is empty.</div>}{mailbox.resultSizeEstimate > mailbox.messages.length && <div className="list-more muted">Showing the 20 newest. {mailbox.resultSizeEstimate - mailbox.messages.length} older messages are hidden.</div>}</Panel>
...
export function Mail({ data, actor, onChanged, onAction }) {
  ...
  return <><PageHead title="Local Mail" subtitle="Mail that stays in this WorldFixture instance. Read it through IMAP and send it through SMTP." command="SMTP submission · IMAP4rev1"/>
    ...
    <Panel title="Local mailboxes · 20 newest" tools={<FolderTabs folder={folder} setFolder={setFolder} inboxCount={folders.inbox?.exists ?? 0} sentCount={folders.sent?.exists ?? 0}/>}>{mailbox.messages?.length ? mailbox.messages.map((message) => <ContentDetails key={`${mailbox.mailbox}:${message.uid ?? message.seq}`} endpoint={`/api/provider/mail-message?mailbox=${encodeURIComponent(mailbox.mailbox)}&uid=${message.uid}`} onReply={() => reply(message)} summary={<><span><strong>{message.headers.subject ?? "Mail message"}</strong><small>{folder === "sent" ? message.headers.to : message.headers.from}</small></span><time>{message.headers.date ?? "—"}</time></>}/>) : <div className="empty">This IMAP folder is empty.</div>}{mailbox.exists > mailbox.messages.length && <div className="list-more muted">Showing the 20 newest. {mailbox.exists - mailbox.messages.length} older messages are hidden.</div>}</Panel>
...
export function Code({ data, actor, onChanged, onAction }) {
  ...
  return <><PageHead title="Code" subtitle="GitHub repositories and issues through the real REST API." command="GitHub REST API"/>
    ...
    {issues.length > 0 && <Panel title={`Open issues · ${issues.length}`}>{issues.map((issue) => { const repository = issue.repository?.full_name ?? issue.repository_url?.split("/repos/").at(-1); return <ContentDetails key={issue.id ?? issue.url} initialText={issue.body} endpoint={`/api/provider/github-issue?repository=${encodeURIComponent(repository ?? "")}&number=${issue.number}`} summary={<><span><strong>{issue.title}</strong><small>{repository} · #{issue.number} · {issue.user?.login ?? "—"}</small></span><time>{issue.updated_at ? new Date(issue.updated_at).toLocaleDateString() : "—"}</time></>}/>; })}</Panel>}
