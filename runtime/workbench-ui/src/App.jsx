...
export function App() {
  ...
  const screens = {
    Overview: <Overview data={data} setScreen={navigate} onRefresh={load} onReset={() => setResetOpen(true)} onAction={onAction}/>,
  ...
  return <>
    ...
    <div className="app-shell"><Sidebar data={data} screen={activeScreen} setScreen={navigate} onGuide={() => setGuideOpen(true)}/><main className="main"><ConnectionConfirmation key={session?.generation ?? "unmanaged"} session={session} onSession={receiveSession}/>{error && <Notice kind="error">{error}</Notice>}{proof && <Notice>{proof}</Notice>}<div className="screen" key={`${runKey}:${activeScreen}`}>{content}</div></main></div>
    <FirstRunGuide key={runKey} open={guideOpen} onClose={closeGuide} steps={guideSteps(data, evidence, { actor })} setScreen={navigate}/>
