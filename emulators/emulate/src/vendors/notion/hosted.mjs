...
function pageBody(markdown) {
  return String(markdown ?? "").split("\n").map((line) => {
    ...
    if (/^- \[[ xX]\] /.test(line)) return `<p class="todo">${line[3].toLowerCase() === "x" ? "☑" : "☐"} ${escapeHtml(line.slice(6))}</p>`;
    if (/^[-*] /.test(line)) return `<p class="list">• ${escapeHtml(line.slice(2))}</p>`;
    if (/^\d+\. /.test(line)) return `<p class="list">${escapeHtml(line)}</p>`;
    return line ? `<p>${escapeHtml(line)}</p>` : "<div class=\"space\"></div>";
  }).join("");
}

function document({ title, markdown, workspace }) {
  return `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${escapeHtml(title)} · ${escapeHtml(workspace)}</title><style>
  ...
  </style></head><body><header><span class="mark">N</span><b>${escapeHtml(workspace)}</b><span>/</span><span>${escapeHtml(title)}</span></header><main class="page">${pageBody(markdown)}</main></body></html>`;
}
